...
class Dimension {
  ...
  Dimension operator -(Dimension other) => Dimension(width - other.width, height - other.height);
  ...
  /// [double]).
  Dimension operator *(double operand) => Dimension(width * operand, height * operand);
  ...
  Vector2 topCenter(Vector2 origin) => Vector2(origin.x + width / 2.0, origin.y);
  ...
  Vector2 centerLeft(Vector2 origin) => Vector2(origin.x, origin.y + height / 2.0);
  ...
  ///
  /// See also [Box.center].
  Vector2 center(Vector2 origin) => Vector2(origin.x + width / 2.0, origin.y + height / 2.0);

  /// The Vector2 to the center of the right edge of the rectangle described by the
  /// given Vector2 (which is interpreted as the top-left corner) and this size.
  ///
  /// See also [Box.centerLeft].
  Vector2 centerRight(Vector2 origin) => Vector2(origin.x + width, origin.y + height / 2.0);
  ...
  ///
  /// See also [Box.bottomLeft].
  Vector2 bottomCenter(Vector2 origin) => Vector2(origin.x + width / 2.0, origin.y + height);
  ...
  Vector2 bottomRight(Vector2 origin) => Vector2(origin.x + width, origin.y + height);
  ...
  /// right edges.
  bool contains(Vector2 vec) => vec.x >= 0.0 && vec.x < width && vec.y >= 0.0 && vec.y < height;

  /// Constrains this with given constraints.
  Dimension constrainBy(Constraints constraints) => constraints.constrainDimension(this);
  ...
  static Dimension? lerp(Dimension? a, Dimension? b, double t) {
    if (b == null) {
      ...
      if (a == null) {
        ...
        return Dimension(_lerpDouble(a.width, b.width, t), _lerpDouble(a.height, b.height, t));
      }
    }
  }
  ...
  @override
  String toString() => 'Dimension(${width.toStringAsFixed(1)}, ${height.toStringAsFixed(1)})';
}
...
class Box {
  ...
  factory Box.fromHandle(Vector2 origin, HandlePosition handle, double width, double height) {
    switch (handle) {
      ...
      case HandlePosition.left:
        return Box.fromLTWH(origin.x - width, origin.y - height / 2, width, height);
      case HandlePosition.top:
        return Box.fromLTWH(origin.x - width / 2, origin.y - height, width, height);
  ...
  static const Box largest = Box.fromLTRB(-_giantScalar, -_giantScalar, _giantScalar, _giantScalar);
  ...
  Box shift(Vector2 vec) => Box.fromLTRB(left + vec.x, top + vec.y, right + vec.x, bottom + vec.y);
  ...
  Box intersect(Box other) => Box.fromLTRB(math.max(left, other.left), math.max(top, other.top),
  ...
  /// Boxes include their top and left edges but exclude their bottom and
  /// right edges.
  bool contains(Vector2 vec) => vec.x >= left && vec.x < right && vec.y >= top && vec.y < bottom;
