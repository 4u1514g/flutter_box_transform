...
class BoxTransformer {
  ...
  static RawResizeResult resize(
    ...
    final Flip currentFlip =
        !allowFlipping ? Flip.none : getFlipForRect(initialRect, delta, handle, resizeMode);

    // This sets the constraints such that it reflects flipRect state.
    if (allowFlipping && (constraints.minWidth == 0 || constraints.minHeight == 0)) {
      ...
      constraints = Constraints(
        minWidth: constraints.minWidth == 0 ? -constraints.maxWidth : constraints.minWidth,
        minHeight: constraints.minHeight == 0 ? -constraints.maxHeight : constraints.minHeight,
    ...
    if (clampingRect.width < initialRect.width || clampingRect.height < initialRect.height) {
      return ResizeResult(
        rect: initialRect,
        oldRect: initialRect,
        flip: initialFlip,
    ...
    double newWidth = (result.rect.width).clamp(100, 400);
    final Box newRect = Box.fromLTWH(result.rect.left, result.rect.top, newWidth,
        ratio == null ? result.rect.height : newWidth / ratio);
  ...
  static ({
    ...
    if (handle.influencesHorizontal) {
      if (rect.width <= initialRect.width && rect.width == constraints.minWidth) {
        minWidthReached = true;
      }
      if (rect.width >= initialRect.width &&
          (rect.width == constraints.maxWidth || rect.width == clampingRect.width)) {
        maxWidthReached = true;
      }
    }
    if (handle.influencesVertical) {
      if (rect.height <= initialRect.height && rect.height == constraints.minHeight) {
        minHeightReached = true;
      }
      if (rect.height >= initialRect.height &&
          (rect.height == constraints.maxHeight || rect.height == clampingRect.height)) {
