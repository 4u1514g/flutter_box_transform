...
class _TransformableBoxState extends State<TransformableBox> {
  ...
  void onHandlePanUpdate(DragUpdateDetails event, HandlePosition handle) {
    ...
    final UIResizeResult result =
        controller.onResizeUpdate(event.localPosition, handle, ratio: widget.ratio);
        ...
        children: [
          ...
          if (widget.resizable)
            for (final handle in HandlePosition.corners.where((handle) =>
                widget.visibleHandles.contains(handle) || widget.enabledHandles.contains(handle)))
              CornerHandleWidget(
                key: ValueKey(handle),
